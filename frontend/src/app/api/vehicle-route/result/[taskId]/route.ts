...
export async function GET(_req: Request, { params }: { params: { taskId: string } }) {
  ...
  try {
    const upstream = await projectBackendFetch(`/api/vehicle-route/result/${encodeURIComponent(params.taskId)}`, {
      method: 'GET',
      userId: user.userId,
      projectMode: 'route',
    });
    ...
    return Response.json({ error: err?.message || 'Failed to fetch vehicle route result' }, { status: 500 });
  }
}
